...
import { Loader2, ArrowLeft, Trash2, Wallet, Hash, Calendar, User, Building, FileText, CheckCircle, MapPin } from 'lucide-react';
import {
    fetchIncomeDetail,
...
// Define the extended type for the loan detail to include the new field
...
const InfoItem: React.FC<{ icon: React.ElementType; label: string; value: any }> = ({ icon: Icon, label, value }) => (
...
const TransactionDetail: React.FC<Props> = ({ transactionId, transactionType, onBack }) => {
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    const [detail, setDetail] = useState<IncomeDetail | ExpenseDetail | LoanDetailWithPayments | null>(null);
    ...
    const handleUpdateStatus = () => {
        ...
        const onConfirmAction = () => {
            const updatePromise = updateLoanStatus(currentLoan.LID, currentLoan)
                .then(() => {
                    refreshTransactions();
                    loadDetails(); // Re-fetch details to show the change
                });
    ...
    const handleDelete = () => {
        ...
        const onConfirmAction = (reason?: string) => {
            ...
            const apiId = transactionType === 'loan' ? (detail as LoanDetailWithPayments).LID : transactionId;

            const deletePromise = deleteTransaction(transactionType, apiId, reason)
                .then(() => {
                    refreshTransactions();
                    onBack();
                });
    ...
    if (loading) {
        return <div className="text-center p-12"><Loader2 className="h-8 w-8 mx-auto animate-spin text-blue-600" /></div>;
    }
    if (error) {
        return <div className="text-center p-12 text-red-600 bg-red-50 rounded-lg">{error}</div>;
    }
    ...
    const renderIncomeDetails = (d: IncomeDetail) => (
        <>
            <div className="text-center p-6 bg-green-50 rounded-xl border border-green-200">
                ...
                <p className="text-4xl font-bold text-green-600 mt-1">{(d.total || 0).toLocaleString('lo-LA')} ₭</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-8">
                ...
                <InfoItem icon={User} label="Employee" value={`${d.employee_fname} ${d.employee_lname} (ID: ${d.user_id})`} />
                <InfoItem icon={User} label="Collector/Customer" value={`${d.collector_fname || ''} ${d.collector_lname || ''} (ID: ${d.member_id})`} />
                <InfoItem icon={Building} label="Market" value={`${d.market_name || ''} (ID: ${d.market_id})`} />
            ...
            <div className="border-t pt-6">
                ...
                {d.photo_url && (
                    <div className="mt-6">
                        ...
                        <img src={d.photo_url} alt="Income" className="rounded-lg border max-w-sm" />
                    </div>
                )}
            </div>
        </>
    ...
    const renderExpenseDetails = (d: ExpenseDetail) => (
        <>
            <div className="text-center p-6 bg-red-50 rounded-xl border border-red-200">
                ...
                <p className="text-4xl font-bold text-red-600 mt-1">{(d.amount || 0).toLocaleString('lo-LA')} ₭</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-8">
                ...
                <InfoItem icon={User} label="Created By" value={`${d.employee_fname} ${d.employee_lname}`} />
             ...
             <div className="border-t pt-6">
                ...
                {d.photo_url && (
                    <div className="mt-6">
                        <p className="text-sm font-medium text-gray-500 mb-2">Attached Photo</p>
                        <img src={d.photo_url} alt="Expense" className="mt-2 rounded-lg border max-w-sm" />
                    </div>
                )}
            </div>
        </>
            ...
            <>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="flex-1 text-center p-6 bg-red-50 rounded-xl border border-red-200">
                        ...
                        <p className="text-2xl font-bold text-red-600 mt-1">{(d.total || 0).toLocaleString('lo-LA')} ₭</p>
                    </div>
                    <div className="flex-1 text-center p-6 bg-green-50 rounded-xl border border-green-200">
                        ...
                        <p className="text-2xl font-bold text-green-600 mt-1">{(d.paid_total || 0).toLocaleString('lo-LA')} ₭</p>
                    </div>
                    <div className="flex-1 text-center p-6 bg-blue-50 rounded-xl border border-blue-200">
                        ...
                        <p className="text-2xl font-bold text-blue-600 mt-1">{outstandingBalance.toLocaleString('lo-LA')} ₭</p>
                    </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-8">
                    ...
                    <div>
                        ...
                        <span className={`inline-flex items-center text-sm font-semibold px-3 py-1 rounded-full ${d.status === 1 ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'}`}>
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between gap-4">
                <div className='flex items-center gap-4'>
                    <button onClick={onBack} className="p-2 rounded-full hover:bg-gray-100"><ArrowLeft className="h-6 w-6" /></button>
                    <h1 className="text-2xl font-bold capitalize">{transactionType} Detail #{transactionId}</h1>
                </div>
            ...
            {/* Sub-Details Section (Only for Income) */}
            {subDetails.length > 0 && (
                <div className="bg-white rounded-xl shadow-sm border p-6">
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full">
                            ...
                            <tbody>
                                {/* Conditionally render rows OR the 'no details' message */}
                                {subDetails.length > 0 ? (
                                    subDetails.map(sub => (
                                    <tr key={sub.member_id} className="border-b">
                                        <td className="px-3 py-2">{sub.Fname} {sub.Lname}</td>
                                        <td className="px-3 py-2 text-right font-medium">{formatCurrency(sub.amount)}</td>
                                        <td className="px-3 py-2">{sub.status}</td>
                                        <td className="px-3 py-2 text-sm text-gray-600">{sub.notes || '-'}</td>
                                    </tr>
                                ))
                                ) : (
                                    // This row renders only when subDetails is empty
                                    <tr>
