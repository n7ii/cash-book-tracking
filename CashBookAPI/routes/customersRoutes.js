...
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    if (roleId === 1) { 
      sql = `SELECT m.*, mk.Mname AS market_name FROM tbmember AS m JOIN tbmarkets AS mk ON m.market_id = mk.MkID WHERE m.role_id = 2 ORDER BY m.Fname ASC`;
    } else { 
      sql = `SELECT m.*, mk.Mname AS market_name FROM tbmember AS m
             JOIN tbmarkets AS mk ON m.market_id = mk.MkID
             JOIN employee_market_assignments AS ema ON m.market_id = ema.market_id
             WHERE m.role_id = 2 AND ema.employee_id = ? ORDER BY m.Fname ASC`;
      values.push(userId);
    }
...
// --- GET ALL LOANS FOR A SPECIFIC CUSTOMER (PROTECTED) ---
router.get('/:id/loans', authMiddleware, async (req, res) => {
  try {
    ...
    const [loans] = await db.query(sql, [customerId]);
