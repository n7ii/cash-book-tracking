...
router.post('/', authMiddleware, async (req, res) => {
  ...
  try {
    const { member_id, total, photo_url = null, notes, market_id, details } = req.body;
    ...
    try {
      const logDetails = JSON.stringify({ total: total, detail_count: details.length, has_photo: !!photo_url});
...
// --- READ: GET ALL COLLECTIONS FOR THE LOGGED-IN USER (with all features) ---
router.get('/', authMiddleware, async (req, res) => {
  try {
    const userId = req.user.userId;
    
    // 1. Get all optional parameters from the URL query
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 20;
    const offset = (page - 1) * limit;
    const searchTerm = req.query.search || '';
    const { startDate, endDate } = req.query; // Get date filters
    ...
    const dataSql = `
      SELECT i.IID, i.total, i.notes, i.created_at, m.Fname AS customer_fname
      FROM tbincome i
      JOIN tbmember m ON i.member_id = m.MID
      ${whereClause} 
